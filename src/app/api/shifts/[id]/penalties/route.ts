...
import { z } from "zod";
import { notifyPenalty } from "@/lib/telegram";
import { createNotificationForEmployee } from "@/lib/notifications";

export async function GET(_: Request, ctx: { params: Promise<{ id: string }> }) {
  const { id } = await ctx.params;
  const items = await prisma.shiftPenalty.findMany({ where: { shiftId: id }, orderBy: { createdAt: "desc" } });
...
export async function POST(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const created = await prisma.shiftPenalty.create({ 
  ...
  try {
    ...
    await createNotificationForEmployee(created.shift.employeeId, {
      type: "penalty",
      title: "Новый штраф",
      message: `Вам назначен штраф ${Number(parsed.data.amount).toFixed(2)} ₽. Причина: ${parsed.data.reason}`,
