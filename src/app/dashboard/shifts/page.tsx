...
export default function ShiftsPage() {
  ...
  const events = useMemo(
    () =>
      (data ?? []).map((s) => ({
        id: s.id,
        title: `${s.employee?.name ?? "Сотрудник"}: ${ruType(s.type)}`,
  ...
  const [form, setForm] = useState<{ type: "MORNING" | "NIGHT" | "CUSTOM"; start: string; end: string }>({ type: "MORNING", start: "09:00", end: "18:00" });
  ...
  const applyTypePreset = (t: "MORNING" | "NIGHT" | "CUSTOM") => {
    if (t === "MORNING") setForm({ type: t, start: "09:00", end: "18:00" });
    else if (t === "NIGHT") setForm({ type: t, start: "20:00", end: "08:00" });
    else setForm({ type: t, start: "09:00", end: "18:00" });
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <div style={{ height: 700 }} className="card p-2">
        <Calendar
          selectable
          localizer={localizer}
          events={events}
        ...
        <Dialog as="div" className="relative z-50" onClose={() => setOpen(false)}>
          ...
          <div className="fixed inset-0 overflow-y-auto">
            <div className="flex min-h-full items-center justify-center p-4">
              ...
              >
                <Dialog.Panel className="w-full max-w-lg rounded bg-white p-5 shadow">
                  <Dialog.Title className="text-lg font-medium">Добавить смену</Dialog.Title>
                  <div className="mt-4 grid grid-cols-2 gap-3">
                    ...
                    <div>
                      <label className="block text-xs text-gray-500">Окончание</label>
                  ...
                  <div className="mt-5 flex justify-end gap-2">
                    <button className="px-3 py-2" onClick={() => setOpen(false)}>Отмена</button>
                    <button className="rounded bg-blue-600 text-white px-3 py-2 hover:bg-blue-700" onClick={createShift}>Сохранить</button>
                  </div>
                </Dialog.Panel>
              </Transition.Child>
            </div>
          ...
          <div className="fixed inset-0 overflow-y-auto">
            <div className="flex min-h-full items-center justify-center p-4">
              <Transition.Child as={Fragment} enter="ease-out duration-200" enterFrom="opacity-0 scale-95" enterTo="opacity-100 scale-100" leave="ease-in duration-150" leaveFrom="opacity-100 scale-100" leaveTo="opacity-0 scale-95">
                <Dialog.Panel className="w-full max-w-3xl rounded bg-white p-5 shadow">
                  <Dialog.Title className="text-lg font-medium">Смена</Dialog.Title>
                  {selectedShift && (
                    <div className="mt-3 grid grid-cols-2 gap-6">
                      <div className="space-y-3">
                        <div className="text-sm">{new Date(selectedShift.startTime).toLocaleString()} — {new Date(selectedShift.endTime).toLocaleString()}</div>
                      ...
                      {isDirector && (
                        <div className="space-y-6">
                          <div>
                            <div className="font-medium mb-2">Штрафы</div>
                            ...
                            <ul className="text-sm space-y-1">
                              {penalties.map((p:any)=> (
                                <li key={p.id} className="flex items-center justify-between border rounded px-2 py-1"><span>-{Number(p.amount).toFixed(2)} ₽ — {p.reason}</span><button className="text-red-600" onClick={async ()=>{ await fetch(`/api/shifts/penalties/${p.id}`, { method:'DELETE' }); refreshAdjustments(selectedShift.id); }}>Удалить</button></li>
                              ))}
                            </ul>
                          </div>
                          <div>
                            <div className="font-medium mb-2">Бонус за смену</div>
                            ...
                            <ul className="text-sm space-y-1">
                              {bonuses.map((p:any)=> (
                                <li key={p.id} className="flex items-center justify-between border rounded px-2 py-1"><span>+{Number(p.amount).toFixed(2)} ₽ — {p.reason}</span><button className="text-red-600" onClick={async ()=>{ await fetch(`/api/shifts/bonuses/${p.id}`, { method:'DELETE' }); refreshAdjustments(selectedShift.id); }}>Удалить</button></li>
                              ))}
                            </ul>
                          </div>
                          <div>
                            <div className="font-medium mb-2">Кальяны</div>
                            <div className="grid grid-cols-3 items-center gap-2">
                            ...
                            <ul className="text-sm space-y-1">
                              {hookahs.map((h:any)=> (
                                <li key={h.id} className="flex items-center justify-between border rounded px-2 py-1"><span>+{h.qty} × {Number(h.amountPer).toFixed(2)} ₽</span><button className="text-red-600" onClick={async ()=>{ await fetch(`/api/shifts/hookahs/${h.id}`, { method:'DELETE' }); refreshAdjustments(selectedShift.id); }}>Удалить</button></li>
                              ))}
                            </ul>
                          </div>
                        </div>
